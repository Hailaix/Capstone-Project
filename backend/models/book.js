...
class Book {
    ...
    */
    static async addBook({ id, title, authors, cover, link }) {
        ...
        INSERT INTO books
        (id, title, authors, cover, link)
        VALUES ($1,$2,$3,$4,$5)
        RETURNING id, title, authors, cover, link`,
            [id, title, joinedAuthors, cover, link]);
     ...
     *  
     *  Returns { id, title, authors, cover, link, reviews}
     *  Where authors is an array of strings,
    ...
    static async get(id) {
        const res = await db.query(`
        SELECT id, title, authors, cover, link
        FROM books
        WHERE id = $1`,
            [id]);
